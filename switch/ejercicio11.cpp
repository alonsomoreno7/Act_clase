...
int main() {
    ...
    switch (operacion) {
        ...
        case '^':
            if (num2 < 0) {
                cout << "Error: Exponente negativo no soportado." << endl;
            } else {
                resultado = 1;
                for (int i = 0; i < num2; i++) {
                    resultado *= num1;  // calcular potencia multiplicando varias veces
                }
                cout << "Resultado: " << resultado << endl;
            }
            break;
        ...
        case '%':
            if (num2 != 0) {
                ...
                cout << "Error: Módulo entre cero no permitido." << endl;
            }
            break;
